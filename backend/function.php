...
$checkResult = $mysqli->query("SELECT id,url,mark,photo_url FROM marks WHERE status=1 ORDER BY mark ASC") or die($mysqli1->error);
       ...
       while ($checkRow2 = $checkResult->fetch_assoc()){
               ...
               echo $myJSON;
                ...
                $checkResult = $mysqli->query("SELECT id, engine,type,power,capacity,start,end,engine_full_name FROM engines WHERE idOfBody='$get_id' AND status=1 ") or die($mysqli1->error);
     while ($checkRow2 = $checkResult->fetch_assoc()){
                ...
                /* foreach ($checkRow2 as $key => $value) {
                  ...
                  $myObj->id = $checkRow2["id"];
                  $myObj->mark =  $checkRow2["engine"] . " ".$checkRow2["type"] . " ". $checkRow2["power"]. "KM " . $checkRow2["capacity"]. "㎤ " .$checkRow2["start"] . "-" . $checkRow2["end"] ;
